...
```  
Adjusted R-Square is 77.35% which is pretty good.  

```{r lm_mod_2}
lm_predict <- predict(simple_linear_model, test_residence_df_scaled)

# measuring the model performance on original price units. 
# model is predicting log of price. Taking exponential 

postResample(pred = exp(lm_predict), obs = test_residence_df$PRICE) 

mod <- cbind(pred = exp(lm_predict), obs = test_residence_df$PRICE)
colnames(mod) <- c('pre','obs')
mod <- data.frame(mod) %>%
  mutate(res = pre-obs)
ggplot(data = mod, aes(x = pre, y = res)) +
  geom_point() +
  labs(title = 'Residuals vs predicted values (original PRICE scale)')


mod <- cbind(pred = lm_predict, obs = test_residence_df_scaled$PRICE_LOG)
colnames(mod) <- c('pre','obs')
mod <- data.frame(mod) %>%
  mutate(res = pre-obs)
ggplot(data = mod, aes(x = pre, y = res)) +
  geom_point() +
  labs(title = 'Residuals vs predicted values (LOG PRICE scale)')
```  
...
```{r rf_1}
set.seed(90898)
rf1 <- randomForest(PRICE_LOG~., data = train_residence_df_scaled, ntree=10)
summary(rf1)
