...
Dataset has 39 variables and 107,237 observations. Create a new df called cleaned version.  
...
'Hot Water Rad (13)', 'Forced Air(1)', Warm Cool(7)' and 'Ht Pump(8)' have majority of residences.
...
Concrete Tile, Wood - FS and Neopren have higher median PRICE compared to other roof types.  
These 3 will be bucketed into other_high and rest in other.  
Built Up(2), Comp Shingle(1), Metal - Sms(6) and Slate(11) are majority of houses roof types. Rest of the
```{r roof_4}
residential_clean_df <- residential_clean_df %>%
...
4 story, 3.5 story unfinished and 3.5 story unifished are grouped and rest in other.
...
Let's group everything else into bucket.  
...
Exceptional-b(10),Exceptional-c(11) and Exceptional-d(12) are one group and rest in othet.  
...
Hardwood(6), Hardwood/Carp(11), wood_floor(3) and carpet(2) are buckets. Rest are analysed.  
...
```
Check out the 113 records which have NA for BATHROOM_TOT
...
Only record with OBJECTID '11154' has bathroom 5 and hf_bathroom as na. Total bath room can be imputed to 5. Rest of the records have na in majority of the columns. These records need to be deleted.
...
```  
There are 66 rows with missing values excluding PRICE columns. Let's drop them.
```{r missing_values}
residential_clean_df <- residential_clean_df %>%
  drop_na(-PRICE)
```  
...
```{r lm_model_1}
set.seed(6969)
simple_linear_model <- train(PRICE~.,data = train_residence_df, method = 'lm')
summary(simple_linear_model)
```  
Only 2 of the features are not significant. Around 60% of variance is explined by this model. This can be taken as baseline.

```{r lm_mod_2}
lm_predict <- predict(simple_linear_model, test_residence_df)
postResample(pred = lm_predict, obs = test_residence_df$PRICE)
```  
The model performance on test data is 62.25%.  

### Linear Regression with repeated cv
```{r lm_rcv_1}
set.seed(59568)
tc <- trainControl(method = 'repeatedcv', number = 10)
lm2_model <- train(PRICE~., data = train_residence_df, method = 'lm',
                   trControl = tc)
summary(lm2_model)
```  
...
set.seed(5684)
tc <- trainControl(method = 'repeatedcv', number = 10)
glm_model <- train(PRICE~., data = train_residence_df, method = 'glm',
                   trControl = tc)
summary(glm_model)

glm_predict_train <- predict(glm_model, train_residence_df)
postResample(pred = glm_predict_train, obs = train_residence_df$PRICE)

glm_predict <- predict(glm_model, test_residence_df)
postResample(pred = glm_predict, obs = test_residence_df$PRICE)
```  
